...
class BaseTradeModel:
    ...
    def __init__(self, trade_model_name, symbol_name, robot_context, robot_config, trade_model_id=None):
        ...
        self.point_value = 1  # 1 USD / 1 point or 10 USD / 1 pip
        self.pos_size_decimal_num = 2  # maximum decimal digits for position size, example: 2 for 1.22, 2.75, 0.05
        self.point_per_pip = 10
        self.min_pos_size = 0.01 # minimum position size (Lot) is 0.01
    ...
    def predict(self, output_type=2):
        ...
        try:
            ...
            self.signal_info['sig_price'] = p_close[-1]
            self.signal_info['sig_open_buy'] = final_sig_open_buy[-1]
            self.signal_info['sig_open_sell'] = final_sig_open_sell[-1]
            self.signal_info['sig_close_buy'] = final_sig_close_buy[-1]
            self.signal_info['sig_close_sell'] = final_sig_close_sell[-1]
            self.signal_info['sig_stop_loss'] = sig_stop_loss[-1]
            self.signal_info['sig_take_profit'] = sig_take_profit[-1]
            self.signal_info['sig_pos_size_pct'] = sig_pos_size_pct[-1]
            self.signal_info['sig_scale_type'] = 0
            self.signal_info['sig_scale_size'] = 0.0
            self.signal_info['sig_stop_type'] = 0
            """
            END: Predict Signal
            """
    ...
    def get_sl_price(self, entry_price, sl_pips, trade_type, tick_size, symbol_digits, point_per_pip):
        price = 0.0
        if sl_pips > 0:
            if trade_type.upper() == cons.BUY:
                price = self.cal_price(entry_price, sl_pips * -1, tick_size, point_per_pip)
            elif trade_type.upper() == cons.SELL:
                price = self.cal_price(entry_price, sl_pips, tick_size, point_per_pip)

            price = round(price, symbol_digits)
        return price

    def get_tp_price(self, entry_price, tp_pips, trade_type, tick_size, symbol_digits, point_per_pip):
        price = 0.0
        if tp_pips > 0:
            if trade_type.upper() == cons.BUY:
                price = self.cal_price(entry_price, tp_pips, tick_size, point_per_pip)
            elif trade_type.upper() == cons.SELL:
                price = self.cal_price(entry_price, tp_pips * -1, tick_size, point_per_pip)

            price = round(price, symbol_digits)
        return price

    def cal_price(self, price, pips, tick_size, point_per_pip):
        new_price = price + (pips * point_per_pip * tick_size)
        return new_price
    # ==============================================================================================================================
